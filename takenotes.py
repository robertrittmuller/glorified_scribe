...
# helper functions
def save_transcription(transcription, file_stub="transcript", output_dir="output"):
    """
    ...
    file_stub (str): The stub for the filename.
...
def process_transcript(transcript):
    ...
    cleaned_transcript = ai.litellm_cleaning(transcript)
    if(cleaned_transcript != None):
        summerized_transcript = ai.litellm_summerization(cleaned_transcript)
...
def main():
    ...
    # Setting up output directory, default as "output"
    ...
    if(args.file == ""):
        ...
        save_transcription(transcription, output_dir=args.output)
    ...
    if ai.num_words(transcription) > 100:
        ...
        if(args.file != ""):
            ...
            if(cleaned_transcript != None):
                save_transcription(cleaned_transcript, file_stub="cleaned", output_dir=args.output)
            if(summerized_transcript != None):
                summerized_transcript += "\n\nDetailed Meeting Notes: \n"
                summerized_transcript += cleaned_transcript
                save_transcription(summerized_transcript, file_stub="summary", output_dir=args.output)
        else:
